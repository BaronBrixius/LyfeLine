...
public class DBM {
    ...
    //How To Define The "creatable" Input: functional interfaces can use the implementation defined by an object, e.g. new User() returns a List of User objects,
    //or it can accept a lambda/method directly which must take in a ResultSet and output an Object, e.g. rs -> rs.getString("Name") will return String objects from the Name field
    ...
    public static <T> void insertIntoDB(DBObject<T>... insert) throws SQLException {   //DON'T INSERT OBJECTS OF DIFFERENT TYPES
        ...
        try {
            for (DBObject<T> t : insert) {
                if (t == null)
                    continue;
                stmt = t.getInsertQuery();              //grabs insert statement as implemented by the passed object and executes it
                stmt.execute();
                //after insertion, get the autogenerated ID and pass it back to the object that was inserted
    ...
    public static <T> void updateInDB(DBObject<T>... update) throws SQLException {     //DON'T INSERT OBJECTS OF DIFFERENT TYPES
        for (DBObject<T> t : update) {
            if (t == null)                          //grabs update statement as implemented by the passed object and executes it
                continue;
            t.getUpdateQuery().execute();
        }
    }

    public static <T> void updateInDB(List<T> update) throws SQLException {            //converts to array so it works with varargs
    ...
    public static <T> void deleteFromDB(DBObject<T>... delete) throws SQLException {           //DON'T INSERT OBJECTS OF DIFFERENT TYPES
        for (DBObject<T> t : delete) {
            if (t == null)                      //grabs delete statement as implemented by the passed object and executes it
                continue;
            t.getDeleteQuery().execute();
        }
    }

    public static <T> void deleteFromDB(List<T> delete) throws SQLException {                  //converts to array so it works with varargs
