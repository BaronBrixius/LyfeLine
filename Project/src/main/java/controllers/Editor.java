...
public abstract class Editor {
	...
	public void initialize() {
		// Set Up the Spinners for Start/End Inputs, would have bloated the .fxml and
		// variable list a ton if these were in fxml
	...
	int setExpansion(FlowPane expandPane, List<VBox> boxesToAddFrom, boolean expanding, int scale) {
		expandPane.getChildren().removeAll(boxesToAddFrom); // clear out the current contents except the expansion
															// button
		...
		if (expanding) { // if expanding, add everything in
			...
			if (scale == 1) // don't try to convert to switch statement unless you're a genius, the overlaps
							// made it ugly when I tried
	...
	boolean validData() {
		Date newStartDate = new Date(startInputs.get(0).getValue(), startInputs.get(1).getValue(),
				startInputs.get(2).getValue(), startInputs.get(3).getValue(), startInputs.get(4).getValue(),
				startInputs.get(5).getValue(), startInputs.get(6).getValue());

		Date newEndDate = new Date(endInputs.get(0).getValue(), endInputs.get(1).getValue(),
				endInputs.get(2).getValue(), endInputs.get(3).getValue(), endInputs.get(4).getValue(),
				endInputs.get(5).getValue(), endInputs.get(6).getValue());
	...
	void populateEndInputs() { // so that end dates can have their display toggled separately, useful for
								// events
	...
	void updateItem() { // sets object's values based on input fields' values
		if (itemInEditor.getImagePath() != null && (!itemInEditor.getImagePath().equals(imageFilePath))) { // deletes
																											// old
																											// picture
																											// if it has
																											// been
		...
		itemInEditor.setStartDate(new Date(startInputs.get(0).getValue(), startInputs.get(1).getValue(),
				startInputs.get(2).getValue(), startInputs.get(3).getValue(), startInputs.get(4).getValue(),
				startInputs.get(5).getValue(), startInputs.get(6).getValue()));

		itemInEditor.setEndDate(new Date(endInputs.get(0).getValue(), endInputs.get(1).getValue(),
				endInputs.get(2).getValue(), endInputs.get(3).getValue(), endInputs.get(4).getValue(),
				endInputs.get(5).getValue(), endInputs.get(6).getValue()));
	}

	boolean hasChanges() { // returns true if any input fields don't match the object's values
		if ((itemInEditor.getImagePath() == null) ? imageFilePath != null
				: !itemInEditor.getImagePath().equals(imageFilePath)) // if item's pic is null, then filename not null
			return true;
		...
		Date readStart = new Date(startInputs.get(0).getValue(), startInputs.get(1).getValue(),
				startInputs.get(2).getValue(), startInputs.get(3).getValue(), startInputs.get(4).getValue(),
				startInputs.get(5).getValue(), startInputs.get(6).getValue());

		Date readEnd = new Date(endInputs.get(0).getValue(), endInputs.get(1).getValue(), endInputs.get(2).getValue(),
				endInputs.get(3).getValue(), endInputs.get(4).getValue(), endInputs.get(5).getValue(),
				endInputs.get(6).getValue());

		return (itemInEditor.getStartDate().compareTo(readStart) != 0
	...
	private void setupTimeInputStartAndEnd(String timeSpinnerLabel, int minValue, int maxValue, int index) { // applies
																												// equivalent
																												// setups
																												// to
																												// both
	...
	// creates spinners to handle dates with appropriate min/max values and invalid
	// input handling
	private void setupTimeInput(String timeSpinnerLabel, int minValue, int maxValue, int index,
			List<Spinner<Integer>> spinnerList, List<VBox> boxList) {
		int initValue = (timeSpinnerLabel.equals("Year")) ? 0 : minValue; // initial value is equal to minimum, except
		...
		valueFactory.setConverter(new StringConverter<>() { // makes spinners revert to default values in case of
															// invalid input
			@Override
			...
			@Override
			public Integer fromString(String string) { // called by spinner to read the value from the box and convert
														// to int
				try {
					if (string == null)
						return initValue;
		...
		spinnerList.get(index).focusedProperty().addListener((observableValue, oldValue, newValue) -> {
			if (!newValue) // the display doesn't restore if invalid info is entered repeatedly, this fixes
							// that
				spinnerList.get(index).cancelEdit(); // note: cancelEdit() is really more like "update display" as
														// implemented. this triggers it upon losing focus
		}); // why this isn't default behavior I'll never know

		// adds each spinner to a VBox underneath its label, to keep the two connected
		// as they move around
