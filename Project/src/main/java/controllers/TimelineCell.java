...
public class TimelineCell {
	...
	public void initialize() {
		...
		ratingBox.setOnMouseMoved(e -> colorStarsByRating((int) Math.ceil(e.getX() * 5 / ratingBox.getWidth()))); // highlight
																													// current
																													// star
																													// and
																													// ones
																													...
																													// the
																													// left
		ratingBox.setOnMouseExited(e -> colorStarsByRating((int) Math.ceil(timeline.getRating()))); // return
																									// highlighting to
																									// normal
	}
	...
	private void setupRatingButton(Polygon button, int index) {
		...
		double angle = 0;
		double distance;
		// calculate position of each point of star, starting from top and going
		// clockwise
		for (int i = 0; i < numPoints * 2; i++) {
			if (i % 2 == 0)
				distance = starSize; // tips stick out further
			else
				distance = starSize / 2; // intersections don't stick out as much, increase number to increase how
											// "sharp" the star is

			button.getPoints().addAll(Math.sin(angle) * distance, // trig to find point position, easier to adjust than
		...
		button.setOnMouseClicked(e -> timeline.addRating(GUIManager.loggedInUser.getUserID(), index + 1)); // click a
																											// star to
																											// submit a
																											// rating
	}
