...
public class TimelineView {
	...
	public void snapshot() throws IOException {
		...
		if (isZoomed()) {
			...
			System.out.println(
					"No zoom printout" + " and height is: " + temp.getHeight() + " and width is: " + temp.getWidth());
	...
	private void setupEventNodes() {
		...
		Collections.sort(eventList); // sort so that earlier events are placed first (longer comes first in case of
										// tie)
	...
	void placeEvent(EventNode newNode, int eventsPlacedCount) {
		...
		if (newNode.getStartColumn() + newNode.getColumnSpan() > timelineGrid.getColumnCount()) // if node goes past the
																								// timeline's end, cut
																								// the end
		...
		for (int i = 0; i < eventsPlacedCount; i++) { // check previous nodes to see if they occupy desired columns
			if (row == eventList.get(i).getRow()
					&& eventList.get(i).getStartColumn() < newNode.getStartColumn() + newNode.getColumnSpan() // if a
																												// previous
																												// node
																												// on
																												// current
	...
	private void scrollHandler(ScrollEvent event) {
		...
		double newScale = event.getDeltaY() > 0 ? oldScale * scaleFactor : oldScale / scaleFactor; // calculate new
																									// scale based on
																									// old
		if (newScale > 100) // max zoom is 100x
			newScale = 100;
		if (newScale < .001) // min zoom is 1/100x
			newScale = .001; // TODO ask client if he's sure he wants no minimum zoom, even at this point
								// each bar is less than a pixel tall, i.e. invisible
		...
		double adjustedHValue = mainScrollPane.getHvalue() * oldScale / newScale // snapshot scrollbar positions before
																					// resizing moves them
				+ hMousePosition * (1 - oldScale / newScale); // adjust snapshots based on mouse position, weighted
																// average of old position and mouse position,
		double adjustedVValue = mainScrollPane.getVvalue() * oldScale / newScale // while zooming in, old position is
																					// ~83% weight (1/1.2) and mouse
																					// position is ~17% (1-(1/1.2))
																					// (assuming scaleFactor is still
																					// 1.2)
				+ vMousePosition * (1 - oldScale / newScale); // while "zooming out away from mouse", mouse position is
																// applied negatively. original position is 120% weight
																// and mouse position is -20%
		...
		mainScrollPane.setHvalue(adjustedHValue); // apply (adjusted) snapshots of scrollbar positions, overriding the
													// above jumping
	...
	private void horizontalScroll(ScrollEvent scrollEvent) { // might wanna add this back in when user is holding a
																// button

		// setup horizontal scroll with mouse wheel
		/*
		 * if (e.getDeltaX() == 0 && e.getDeltaY() != 0) {
		 * mainScrollPane.setHvalue(mainScrollPane.getHvalue() - e.getDeltaY() /
		 * mainScrollPane.getWidth()); }
		 */
	}

	/*
	 * public File fileChooser() { FileChooser fileChooser = new FileChooser();
	 * String format = ".png";
	 * 
	 * fileChooser.setInitialFileName(activeTimeline.getName().replaceAll("\\s+",
	 * "_") + format); //We will add read format from dropdown or use png
	 * fileChooser.getExtensionFilters().addAll( //keep all formats now, easy to add
	 * to the popup new FileChooser.ExtensionFilter("All Images", "*.jpg", "*.jpeg",
	 * "*.png", "*.bmp", "*.gif", "*.wbmp"), new FileChooser.ExtensionFilter("JPG",
	 * "*.jpg"), new FileChooser.ExtensionFilter("JPEG", "*.jpeg"), new
	 * FileChooser.ExtensionFilter("PNG", "*.png"), new
	 * FileChooser.ExtensionFilter("BMP", "*.bmp"), new
	 * FileChooser.ExtensionFilter("GIF", "*.gif"), new
	 * FileChooser.ExtensionFilter("WBMP", "*.wbmp") ); //Show save file dialog File
	 * file = fileChooser.showSaveDialog(GUIManager.mainStage); return file; }
	 * 
	 * 
	 * //Just a placeholder method that creates a image of the snapshot public void
	 * copy(WritableImage temp) throws IOException { BufferedImage fromFXImage =
	 * SwingFXUtils.fromFXImage(temp, null);
	 * System.out.println(fromFXImage.getHeight() + " and width is " +
	 * fromFXImage.getWidth());
	 * 
	 * // Calculate height width , offset int width = fromFXImage.getWidth(); int
	 * height = fromFXImage.getHeight() ; int height2 = (int) (height * 1.20); int
	 * offset = (int) (height * 0.1);
	 * 
	 * // Create another image with new height & width BufferedImage backImage = new
	 * BufferedImage( width, height2, BufferedImage.TYPE_INT_RGB); Graphics2D g =
	 * backImage.createGraphics();
	 * 
	 * // Am setting the color to black to distinguish , otherwise it can be set to
	 * Color.white g.setColor(Color.white); // Fill hte background with color
	 * g.fillRect(0, 0, width , height2); // Now overlay with image from offset
	 * g.drawImage(fromFXImage,0,offset,null); // write to the file
	 * ImageIO.write(backImage, "PNG", fileChooser() );
	 * System.out.println(backImage.getHeight() + " and width is " +
	 * backImage.getWidth()); g.dispose(); }
	 */
